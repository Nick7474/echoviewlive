...
// Custom Safety Icon
const createSafetyIcon = (color: string, iconType: string) => {
  let iconSvg = '';
  switch(iconType) {
    case '소화전': iconSvg = '<path d="M12 22v-5M9 17h6M12 17V7M12 7c2.76 0 5-2.24 5-5H7c0 2.76 2.24 5 5 5Z"/>'; break;
    case 'AED': iconSvg = '<path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/>'; break;
    case '비상벨': iconSvg = '<path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/>'; break;
    case '대피소': iconSvg = '<path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>'; break;
...
const SAFETY_DATA = [
  { id: 1, name: '광명시청 소화전', type: '소화전', address: '광명시 시청로 20', lat: 37.4788, lng: 126.8640, status: '정상' },
  { id: 2, name: '철산역 AED', type: 'AED', address: '철산동 241-1', lat: 37.4762, lng: 126.8670, status: '정상' },
  { id: 3, name: '광명사거리 비상벨', type: '비상벨', address: '광명동 158-6', lat: 37.4792, lng: 126.8540, status: '정상' },
  { id: 4, name: '광명초 민방위 대피소', type: '대피소', address: '광명동 288-1', lat: 37.4822, lng: 126.8585, status: '정상' },
  { id: 5, name: '광명전통시장 소화전', type: '소화전', address: '광명동 150-19', lat: 37.4808, lng: 126.8560, status: '정상' },
];
...
export default function SmartCityMapSafety() {
  ...
  const filteredData = useMemo(() => {
    return SAFETY_DATA.filter(item => selectedTypes.includes(item.type));
  }, [selectedTypes]);
      ...
      >
        ...
        {filteredData.map((item) => (
          <Marker 
            ...
            icon={createSafetyIcon(TYPE_COLORS[item.type] || '#6B7280', item.type)}
          >
            <Popup className="custom-popup">
              <div className="p-1">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-line-normal">
                  <span className="text-[10px] text-gray-400">상태: <span className="text-green-500 font-bold">{item.status}</span></span>
                  <button className="text-[10px] font-bold text-primary hover:underline">길찾기</button>
                </div>
              </div>
            </Popup>
          </Marker>
