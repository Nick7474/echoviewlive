    ...
    <div className="min-h-screen flex flex-col font-sans text-gray-800">
      ...
      >
        ...
        <div className={location.pathname === '/'
          ...
          {/* Logo */}
          <Link to="/" className={`flex items-center gap-3 ${location.pathname === '/' ? 'absolute left-[89px]' : ''}`}>
            <img src="/images/Logo.png" alt="에코뷰" className="h-[42px] w-auto" />
            <div className="flex items-center gap-2">
              <span className="text-[18px] font-light text-[#222] leading-[1.3]" style={{ letterSpacing: '-0.72px' }}>광명시 통합플랫폼</span>
              <span className="text-[22px] font-bold text-[#0f0f0f] leading-[1.2]">ECOVIEW</span>
            </div>
          </Link>

          {/* Desktop Nav */}
          <nav className={`hidden lg:flex items-center h-full ${location.pathname === '/' ? 'absolute left-[calc(50%+120px)] -translate-x-1/2' : 'ml-auto mr-auto translate-x-[120px]'}`}>
            <ul className="flex items-center h-full gap-[100px]">
              {NAVIGATION_MENU.map((menu) => {
                ...
                return (
                  ...
                  >
                    <Link
                      to={menu.path}
                      className={`text-[18px] font-bold h-full flex items-center whitespace-nowrap hover:text-[#017DDD] transition-colors relative ${isMenuOpen ? 'text-[#017DDD]' : 'text-[#212529]'}`}
                    >
                      {menu.title}
                      {isMenuOpen && (
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#017DDD]" />
                      )}
                    </Link>
                  </li>
                );
            ...
            <div className="hidden sm:flex items-center gap-6">
              ...
              >
                ...
                >
                  ...
                  <span className="text-[14px] font-medium text-[#444]">홍길동</span>
                </Link>
              ...
              <button aria-label="알림" className="relative text-gray-500 hover:text-gray-700 transition-colors">
                <Bell size={20} />
                <span className="absolute -top-1 -right-1 w-[14px] h-[14px] bg-[#e53a4c] rounded-full flex items-center justify-center">
                  <span className="text-white font-bold text-[8px] leading-none">1</span>
                </span>
              </button>
            </div>
          ...
          {isFullMenuOpen && (
            ...
            >
              <div className="flex justify-center">
                <div className="grid border-x border-line-normal bg-white translate-x-[110px]" style={{ gridTemplateColumns: 'repeat(6, 200px)' }}>
                  {NAVIGATION_MENU.map((menu) => {
                    ...
                    return (
                      <div
                        ...
                        className={`py-10 px-5 flex flex-col transition-colors duration-200 border-r border-line-normal last:border-r-0 min-h-[280px] ${
                          isHovered ? 'bg-[#017DDD]' : 'bg-white hover:bg-[#e8f4fd]'
                        }`}
                        onMouseEnter={() => setHoveredMenu(menu.title)}
                      >
                          ...
                          {menu.children?.map((child, idx) => {
                            ...
                            if (hasSubChildren) {
                              return (
                                <div key={child.title} className="pt-3 first:pt-0">
                                  <p className={`text-[12px] font-black mb-1.5 uppercase tracking-wide ${isHovered ? 'text-white/70' : 'text-slate-900'}`}>
                                    {child.title}
                                  </p>
                                  <div className="space-y-1">
                                    {child.children?.map((sub) => (
                                      <Link
                                        key={sub.title}
                                        to={sub.path}
                                        className={`block text-[13px] transition-all ${
                                          isHovered
                                            ? 'text-white/90 hover:text-white hover:underline underline-offset-4'
                                            : isActive(sub.path)
                                              ? 'text-[#017DDD] font-bold'
                                              : 'text-gray-600 hover:text-[#017DDD]'
                                        }`}
                                      >
                                        {sub.title}
                                      </Link>
                            ...
                            return (
                              <div key={child.title} className={`${prevHasGroup ? 'pt-3 first:pt-0' : 'pt-3 first:pt-0'}`}>
                                <p className={`text-[12px] font-black uppercase tracking-wide ${isHovered ? 'text-white/70' : 'text-slate-900'}`}>
                                  <Link to={child.path} className={`transition-colors ${isHovered ? 'hover:text-white' : 'hover:text-[#017DDD]'}`}>
                                    {child.title}
                                  </Link>
                                </p>
                              </div>
